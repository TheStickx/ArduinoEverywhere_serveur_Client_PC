...
namespace DroneCMD
{
    ...
    public class AsynchronousClient
    {
        ...
        public delegate void ChangementStatutServerHandler();
        public ChangementStatutServerHandler ChangementStatutServer;


        // pour information
        public int ByteSend=0, ByteReceive=0, NbrBlueTooth=0;
        private bool bIsServerResponding, bWasServerResponding;
        public bool bServerResponding;
        ...
        public AsynchronousClient(Form NecessairePourLaSuite)
        {
            ...
            TimerRefresh = new System.Timers.Timer();
            TimerRefresh.Elapsed += new ElapsedEventHandler(OnTimedRefresh);
        ...
        private void OnTimedRefresh(object source, ElapsedEventArgs e)
        {

            // bIsServerResponding, bServerResponding;   bWasServerResponding
            bServerResponding = bIsServerResponding; // pour que le monde sache
            if (bIsServerResponding != bWasServerResponding)
            {
                // changement de statut => invoque évenement
                PourRetourVersGUI.Invoke(ChangementStatutServer, new object[] { });
            }
            bWasServerResponding = bIsServerResponding;  // pour  la prochaine
            bIsServerResponding = false;

            try
            {
                if (clientAndruino != null && clientAndruino.Connected) SendPingApp();
                else StartClient();
            }
            catch (Exception exp)
            {
                ThrowError(exp.ToString());
                StartClient();
            }
        }
        ...
        public void ProcessReception(String Reception)
        {
            ...
            while ((iDebutEtiquette >= 0) && (iDebutContenu > 0) && (iFinContenu > 0))
            {
                ...
                if (Etiquette == "NbrBT")
                {
                    int iTempNbrBlueTooth;
                    if ( !Int32.TryParse(Contenu, out iTempNbrBlueTooth)) iTempNbrBlueTooth = 0;

                    // si le nombre de blue tooth change on balance un évenement
                    if (NbrBlueTooth != iTempNbrBlueTooth)
                    {
                        NbrBlueTooth = iTempNbrBlueTooth;
                        PourRetourVersGUI.Invoke(ChangementStatutServer, new object[] {});
                    } else
                    {
                        NbrBlueTooth = iTempNbrBlueTooth;
                    ...
                    // le serveur réponds aux pings
                    bIsServerResponding = true;  
                }
